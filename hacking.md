# Hacking koto-cli
Koto-cli is set up to scrape one platform - currently anikototv.to. Supporting multiple sources at a time would require more changes than we (the maintainers) find worth doing, for this reason any feature request asking for a new site is rejected.

However koto-cli being open-source and the pirate anime streaming sites being so similar you can hack koto-cli to support any site that follows a few conventions.
...
- writing regexes
You'll also need web browser with a debugger and environment that can run unmodified koto-cli

## The scraping process
The following flowchart demonstrates how koto-cli operates from a scraping standpoint:

![image](.assets/koto-cli-scraping-flow.png)
...
Once you figured out how searching works, you'll have to replicate it in the `search_anikoto` function.
...
You'll have to change some variables in the process (eg. provider_base) too.

If you have done everything correctly, you can run `koto-cli`, query your site of choice and select from the responses.
Then koto-cli should fail without a message.
If it fails with `No results found!` you have debugging to do.
Running koto-cli with `sh -x` is a good way to debug.
...
This might be a series overview page (like with anikototv.to) or there might not be such, but the episode pages have links to all episodes.

You'll have to edit the `episodes_list_anikoto` function that downloads this list of urls.
...
Then koto-cli should fail with `Episode not released!`

### Getting the player embed
After selecting an episode, the next step is to load its page and extract the embed(s).
In case you can get them without loading the episode page, replace from the `get the embed urls...` part of the code to the removal of the cache dir with a single call to `anikoto_server_links` and load its output into `links`.
...
From here they are separated and parsed by `anikoto_server_links` to get the media links.

If there's only one embed source, the `generate links..` block can be reduced to a single call to `anikoto_server_links`.
...
This is done in the script with the `anikoto_server_links` function.
...
The output of the `anikoto_server_links` function needs to be concatenated into the `links` variable - with a single call if there's only one source, or with the asynchronous mode if there are more.
From here the `get_episode_url` function will continue with quality selection which you need not to alter.

## Other functionality
Assuming you completed all the necessary modifications, koto-cli should completely work for you now.
...
There might be cases that can't be covered by the current structure of koto-cli, but still it works for most sites as I've observed.

## UX Spec

There also exists a UX spec if you want to replicate the koto-cli user experience in a fresh codebase:
![image](.assets/koto-cli-ux-spec.png)
